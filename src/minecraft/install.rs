...
impl MinecraftInstallation {
    ...
    async fn install_resources(&self) -> Result<Vec<(Resource, BetterPathBuf)>> {
        let mut res = vec![];
    ...
    pub(crate) fn install_libraries(&self, libraries: &Vec<Library>, always_download_nohash: bool) -> Result<Vec<(Resource, BetterPathBuf)>> {
        let mut res = vec![];
        ...
        for lib in libraries {
            ...
            match lib {
                ...
                Library::VanillaForgeAndNeo(l) => {
                    res.push((l.downloads.artifact.res.clone(), lib_path.clone() / &l.downloads.artifact.path))
                }
                Library::VanillaNatives(l) => {
                    if let Some(os) = l.natives.get(&get_os()) {
                        let artifact = l.downloads.classifiers.get(os).unwrap();
                        res.push((artifact.res.clone(), lib_path.clone() / &artifact.path))
                    }
                }
                Library::BaseOnly(l) => {
                    res.push((Resource {
